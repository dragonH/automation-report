...
const screenAndUploadToS3 = async (user: string, type: string, page: Page) => {
  const s3 = new AWS.S3();
  const time = moment().format("YYYYMMDDHHmm");
  const screenshot = await page.screenshot();
  const objectDetail = await s3
    .upload({
...
const sendMail = async (results: Iresults, error: Error | null) => {
  const sns = new AWS.SNS();
  const { TopicArn } = await sns
    .createTopic({ Name: "automation-report" })
    .promise();
  let message = "";
  const { placeResult, statusResult, succeedResult, errorResult } = results;
  if (placeResult && statusResult && succeedResult) {
    message = `
        ...
        `;
    const sendMailResult = await sns
      .publish({ TopicArn, Message: message })
      .promise();
...
export const autoReport = async () => {
  ...
  try {
    const { user, email, password } = process.env;
    ...
    ********    *******       *
    ...
    await page.waitForSelector('input[value="辦公室 (ECV office)"]', {
      timeout: 60000
    });
